...
function loadData(pagenumber) {
    var url = '/Admin/StoreTimeSheetData_Paging';
    var data = JSON.stringify({ "status": 10, "storeid": $("#StoreID").val(), "pagenumber": pagenumber, "pagesize": pagesize });
...
function LoadGridView(result) {
    if (result.Item1 > 0) {
        ...
        $.each(outputt, function (key, item) {
            ...
            html += '<td class="text-center">' + item.FromDayName + '</td>';
            html += '<td class="text-center">' + item.ToDayName + '</td>';
            html += '<td class="text-center">' + item.FromTimeString + '</td>';
            html += '<td class="text-center">' + item.ToTimeString + '</td>';
            html += '<td class="text-center">' + item.StepTime + '</td>';
...
function Add() {
    ...
    formData.append("FromDay", $('#FromDaySelect option:selected').val())
    formData.append("ToDay", $('#ToDaySelect option:selected').val())
    formData.append("FromTime", $('#FromTime').val())
    formData.append("ToTime", $('#ToTime').val())
    formData.append("StepTime", $('#StepTime').val())
    formData.append("Status", $('#Status option:selected').val())
    $.ajax({
        type: "POST",
        url: '/Admin/StoreTimeSheetData_Insert',
        data: formData,
        dataType: 'json',
        contentType: false,
        processData: false,
...
function getbyID(iD) {
    var url = '/Admin/StoreTimeSheetData_GetByID';
...
function ShowDetails(result) {
    ...
    $('#FromDaySelect').val(result.FromDay);
    $("#ToDaySelect").val(result.ToDay);
    $("#FromTime").val(result.FromTimeString);
    $("#ToTime").val(result.ToTimeString);
    $("#StepTime").val(result.StepTime);
...
function Update() {
    ...
    $.ajax({
        type: "POST",
        url: '/Admin/StoreTimeSheetData_Update',
        data: formData,
        dataType: 'json',
        contentType: false,
        processData: false,
...
function Delele(ID) {
    ShowConfirm('توجه', 'آیا مایل به حذف رکورد می باشید ؟',
        function myfunction() {
            var url = '/Admin/StoreTimeSheetData_Delete'
            var data = JSON.stringify({ "id": ID });
